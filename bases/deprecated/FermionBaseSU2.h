...
public:
	 ...
	 * \param orbital : orbital index
	 */
	Operator Tx (std::size_t orbital=0) const;

	/**
	 * Isospin y-component times i
	 * \param orbital : orbital index
	 */
	Operator iTy (std::size_t orbital=0) const;
...
//	assert(N_orbitals>=1);
	...
	typename Symmetry::qType Q={1}; //empty occupied state
	Eigen::Index inner_dim = 2;
	...
	Id_vac = Operator({1},basis_1s);
	Zero_vac = Operator({1},basis_1s);
...
cc (std::size_t orbital) const
{
	...
	else
	{
		Operator out;
		bool TOGGLE=false;
		if(orbital == 0) { out = Operator::outerprod(p_1s,Id_1s,{1}); TOGGLE=true; }
		else
		{
			if( orbital == 1 ) { out = Operator::outerprod(Id_1s,p_1s,{1}); TOGGLE=true; }
			else { out = Operator::outerprod(Id_1s,Id_1s,{1}); }
		}
		for(std::size_t o=2; o<N_orbitals; o++)
		{
			if(orbital == o) { out = Operator::outerprod(out,p_1s,{1}); TOGGLE=true; }
			else if(TOGGLE==false) { out = Operator::outerprod(out,Id_1s,{1}); }
			else if(TOGGLE==true) { out = Operator::outerprod(out,Id_1s,{1}); }
		}
		return out;
	}
}
...
SiteOperatorQ<Sym::SU2<Sym::SpinSU2>,Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> > FermionBase<Sym::SU2<Sym::SpinSU2> >::
Tx (std::size_t orbital) const
{
	return 0.5*(cdagcdag(orbital)+cc(orbital));
}

SiteOperatorQ<Sym::SU2<Sym::SpinSU2>,Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> > FermionBase<Sym::SU2<Sym::SpinSU2> >::
iTy (std::size_t orbital) const
{
	return 0.5*(cdagcdag(orbital)-cc(orbital));
}
...
HubbardHamiltonian (const ArrayXd &U, const ArrayXd &Uph, const ArrayXd &Eorb, const ArrayXXd &t, const ArrayXXd &V, const ArrayXXd &Vz, const ArrayXXd &Vxy, const ArrayXXd &J) const
{
	...
	for (int j=0; j<i; ++j)
	{
		...
		if (V(i,j) != 0.)
		{
			Mout += V(i,j) * (Operator::prod(n(i),n(j),{1}));
		}
		if (Vz(i,j) != 0.)
		{
			Mout += Vz(i,j) * (Operator::prod(Tz(i),Tz(j),{1}));
		}
		if (Vxy(i,j) != 0.)
		{
			Mout += 0.5 * Vxy(i,j) * pow(-1.,i+j) * (Operator::prod(cc(i),cdagcdag(j),{1}) + Operator::prod(cdagcdag(i),cc(j),{1}));
		}
		if (J(i,j) != 0.)
		{
