...
int main (int argc, char* argv[])
{
	...
	else {clockwise="counter-clockwise";}
	...
	if (SU2)
	{
		...
		auto gs = firstEigenStates(num_eigenstates, DMRG_SU2, H_SU2, g_SU2, qarray<1>{Dtot});
		...
		printCorrs(gs[n].state,H_SU2); cout << endl;
		auto copy_state = gs[n].state;
		copy_state.set_pivot(-1);
		for (size_t l=1; l<=L; l++)
		{
			rotate(copy_state,Swap_SU2,CLOCKWISE);
			cout << "Rotated " << clockwise << " by " << l*360./L << " degree:" << endl;
			cout << copy_state.info() << endl << copy_state.test_ortho() << endl << endl;
	...
	if (U1)
	{
		...
		auto gs = firstEigenStates(num_eigenstates, DMRG_U1, H_U1, g_U1, qarray<1>{M});
		...
		printCorrs(gs[n].state,H_U1); cout << endl;
		auto copy_state = gs[n].state;
		copy_state.set_pivot(-1);
		for (size_t l=1; l<=L; l++)
		{
	...
	if (U0)
	{
		...
		auto gs = firstEigenStates(num_eigenstates, DMRG_U0, H_U0, g_U0, qarray<0>{});
		...
		printCorrs(gs[n].state,H_U0); cout << endl;
		auto copy_state = gs[n].state;
		copy_state.set_pivot(-1);
		for (size_t l=1; l<=L; l++)
		{
