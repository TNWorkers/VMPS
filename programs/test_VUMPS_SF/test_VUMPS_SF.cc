...
//	}
	...
	GlobParams.tol_var = 1e-4;
	GlobParams.tol_state = 1e-2;
	GlobParams.min_iterations = 10;
	GlobParams.max_iterations = 100;
	GlobParams.Dinit = 10;
	
	typedef VMPS::HeisenbergXXZ HEISENBERG_U1;
	HEISENBERG_U1 Heis_U1(L,{{"Jxy",Jxy},{"Bx",Bx},{"Jz",Jz},{"OPEN_BC",false}}); //{"Bx",Bx},
...
//		DMRG_U1.set_verbosity(DMRG::VERBOSITY::SILENT);
		DMRG_U1.edgeState(Heis_U1, g_U1, {});
		
		int N = 51;
		ofstream Filer("SF_U0.dat");
		
		auto Sz = Heis_U1.Sz(0);
		auto Sx = Heis_U1.Sx(0);
		double Szavg = avg(g_U1.state, Heis_U1.Sz(0), g_U1.state);
		double Sxavg = avg(g_U1.state, Heis_U1.Sx(0), g_U1.state);
		Sz.scale(1.,-Szavg);
		Sx.scale(1.,-Sxavg);
		
		cout << "<Sz>=" << avg(g_U1.state, Sz, g_U1.state) << ", shifted by " << Szavg << endl;
		cout << "<Sx>=" << avg(g_U1.state, Sx, g_U1.state) << ", shifted by " << Sxavg << endl;
		
		ArrayXcd SFzz = g_U1.state.structure_factor(Sz,Sz);
//		ArrayXcd SFxx = g_U1.state.structure_factor(Sx,Sx);
		ArrayXcd SFxx(SFzz.rows()); SFxx=0;
		
		for (int i=0; i<SFzz.rows(); ++i)
		{
//			cout << i << "\t" << SFzz(i) << "\t" << SFxx(i) << endl;
			Filer << i*2.*M_PI/(SFzz.rows()-1) << "\t" 
			      << SFzz(i).real() << "\t" << SFzz(i).imag() << "\t" 
			      << SFxx(i).real() << "\t" << SFxx(i).imag() << endl;
		}
		Filer.close();
		
		cout << "Sz(k=π)=" << g_U1.state.structure_factor(Sz,Sz,M_PI,M_PI,1,false) << endl;
		cout << "Sz(k=0)=" << g_U1.state.structure_factor(Sz,Sz,0.,0.,1,false) << endl;
//		cout << "Sx(k=π)=" << g_U1.state.structure_factor(Sx,Sx,M_PI,M_PI,1,false) << endl;
//		cout << "Sx(k=0)=" << g_U1.state.structure_factor(Sx,Sx,0.,0.,1,false) << endl;
		
		N = 50;
		
		HEISENBERG_U1 Htmp(N+1,{{"Jxy",Jxy},{"Jz",Jz},{"Bx",Bx},{"OPEN_BC",false}}); // ,{"Bx",Bx}
		ArrayXd SzSzR(N);
		ArrayXd SxSxR(N);
		for (size_t l=0; l<N; l++)
		{
			SzSzR(l) = avg(g_U1.state, Htmp.SzSz(0,l), g_U1.state) - Szavg*Szavg;
			SxSxR(l) = avg(g_U1.state, Htmp.SxSx(0,l), g_U1.state) - Sxavg*Sxavg;
		}
		...
		for (size_t l=0; l<N; l++)
		{
			SzSzL(l) = avg(g_U1.state, Htmp.SzSz(l,0), g_U1.state) - Szavg*Szavg;
			SxSxL(l) = avg(g_U1.state, Htmp.SxSx(l,0), g_U1.state) - Sxavg*Sxavg;
		}
		...
		for (size_t i=0; i<2*N; i++)
		{
			...
			for (size_t l=0; l<N; l++)
			{
				if (l!=0)
				{
					Szk(i) += SzSzR(l) * exp(-1.i*k*static_cast<double>(l));
					Sxk(i) += SxSxR(l) * exp(-1.i*k*static_cast<double>(l));
				}
			}
			for (size_t l=0; l<N; l++)
			{
				if (l!=0)
				{
					Szk(i) += SzSzL(l) * exp(+1.i*k*static_cast<double>(l));
					Sxk(i) += SxSxL(l) * exp(+1.i*k*static_cast<double>(l));
				}
			}
		}
		
		ofstream FilerFT("SF_FT.dat");
		for (size_t i=0; i<2*N; i++)
		{
			double k = i * 2.*M_PI/(2*N);
